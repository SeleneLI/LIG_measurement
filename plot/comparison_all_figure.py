...
for row in reader:
    ...
    for TSP in row:
        ...
        Date_str = str(Date.year)+str('%02d'%Date.month)+str('%02d'%Date.day)
        if int(start_date) <= int(Date_str) <= int(end_date) :
           TSPs_all.append(datetime.datetime.fromtimestamp(int(TSP)))

    break

# Obtainning the Information from '-Negative-LISP.csv' for all the Timestamps
mapping_lists = [] #for each map resolver
mapping_overall_mapping = [] #combined all the map resolvers
MR_counter = 0
for map_resolver in map_resolvers:
   counter_TSP = 0
   mapping_list = []

   while counter_TSP < len(TSPs_all):
       print(counter_TSP)
       counter_mapping = 0
       x = []
       ...
       for row in reader:
           if row[0] == '':
               TSP_position = row.index(str(int(TSPs_all[counter_TSP].timestamp())))
               continue
           #del row[0]
           if row[TSP_position] != '' :
               counter_mapping += 1
               if MR_counter == 0 :
                   ...
                   if row[0] not in mapping_overall_mapping[counter_TSP] :
                       mapping_overall_mapping[counter_TSP].append(row[0])

       counter_TSP +=1
       mapping_list.append(counter_mapping)
       if MR_counter == 0 :
            mapping_overall_mapping.append(x)
   mapping_lists.append(mapping_list)
   MR_counter += 1
...
# Plot the overall LISP Replies for all the Timestamps
mapping_overall_mapping_number = []
for x in mapping_overall_mapping:
    mapping_overall_mapping_number.append(len(x))


Zero_position = mapping_overall_mapping_number.index(2)
mapping_overall_mapping_number[Zero_position] = mapping_overall_mapping_number[Zero_position-1]
...
plt.plot_date(dates_fixed_hour, mapping_list_LISPmon, ls='-', marker='o' , label = 'LISPmon'  ,  ms= 8)

# Plot the LISP Replies of the project with fixed hour
plt.plot_date(dates_fixed_hour, mapping_overall_fixed_number, ls='-', marker='^' , label = 'LISP-Views' , ms= 8)
...
plt.ylabel('Number of LISP Map-Reply' , fontsize=20)

plt.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc=3, ncol=2, borderaxespad=0.8) # , mode="expand"
...
plt.plot_date(dates, mapping_overall_mapping_number,color= 'r', ls='-', marker='' , label = 'overall Mapping' )
plt.plot_date(dates, mapping_overall_number,color= 'k', ls='--', marker='' , label = 'LISP Map-Reply' )
...
plt.ylabel('Number of Reply'  , fontsize=20)
#plt.set_yscale('log')
plt.yscale('log')

plt.legend(bbox_to_anchor=(0., 0.98, 1., .102), loc=3, ncol=2, borderaxespad=0.8) # , mode="expand"
plt.gcf().autofmt_xdate()
plt.xticks(np.arange(min(dates), max(dates), 2),fontsize=15, fontname="Times New Roman")
plt.yticks(fontsize=15, fontname="Times New Roman")
...
plt.savefig(os.path.join(FIGURE_PATH, 'LISPmon_comparison_all.eps'), dpi=300, transparent=True  )#, bbox_extra_artists=(lgd) # you can change the name, just an example
