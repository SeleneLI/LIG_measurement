...
def scan( Name , Start , End , Timestamp ,port_source):
    ...
    while ipaddress.ip_address(Start) <= ipaddress.ip_address(End):
        ...
        try:
            ...
            #display_information.display(None , 0, Start, map_resolver, ip_my , Timestamp=Timestamp , rtt= None , sender_addr= None , resolver_num=4 )
        ...
        if len(map_reply.records[0].locator_records) != 0:
            display_information.display(map_reply , 1 , Start , map_resolver , ip_my , rtt , sender_addr , Timestamp , resolver_num=4 )
            #print('processing ' + map_resolver + '  ' + Name + '...' + Start)
        # negative Map Reply
        else:
            display_information.display(map_reply, -1, Start , map_resolver , ip_my , rtt , sender_addr , Timestamp , resolver_num=4)
